...
if (!$creation_step) { //show edit_project_details so the user can insert the name of the project
	...
	if (!empty($refresh_page_without_creation_step)) {
	...
	else if (!empty($_POST) && !empty($status)) {
		$call_edit_project_details = false;
		
		//This code was copied and updated from the view/presentation/edit_project_details.php
		$msg = !empty($extra_message) ? $extra_message : ""; //only add this message if the there are extra messages, otherwise the system will show a successfully message in creation_step 2
	...
	if ($call_edit_project_details) {
		...
		$project_exists = (!empty($_POST) && !empty($is_previous_existent_project)) || (empty($_POST) && !empty($is_existent_project));
		...
		$main_content = '<div class="top_bar create_project_top_bar popup_with_iframe_left_popup_close popup_with_iframe_popup_close_button' . ($popup ? ' in_popup' : '') . '">
			<header>
				...
				<ul>
					...
					<li class="cancel button" data-title="Cancel"><a class="active" href="javascript:void(0)"' . (!empty($is_existent_project) ? ' onClick="cancel()' : '') . '">Cancel</a></li>
				</ul>
			</header>
		</div>' . $main_content;
	}
	...
	if ($creation_step == 3) { //show list with programs in our store so the user can install them
		...
		if (!$step) {
			...
			if (!empty($is_last_step_successfull))
				$status_message = "";
		}
	}
		...
		<div class="message">
			...
			' . (!empty($msg) ? '<div class="info">' . $msg . '</div>' : '') . '
		...
		if (!empty($msg))
			$status_message = $msg . (!empty($status_message) ? "<br/>$status_message" : "");
	}
...
var project_post_data = ' . (isset($project_post_data) ? json_encode($project_post_data) : "null") . ';
var project_exists = ' . (!empty($project_exists) ? "true" : "false") . ';
</script>';
