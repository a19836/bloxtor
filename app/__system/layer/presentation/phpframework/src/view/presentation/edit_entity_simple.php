...
$confirm_save = $obj_data["code"] && $cached_modified_date != $file_modified_time;
...
if ($obj_data) {
	$code_exists = !empty(trim($obj_data["code"]));
	...
	if ($regions_blocks_list)
		foreach ($regions_blocks_list as $rbl) {
			if (!is_array($template_region_blocks[ $rbl[0] ]))
				$template_region_blocks[ $rbl[0] ] = array();
			
			$template_region_blocks[ $rbl[0] ][] = $rbl;
		}
	...
	$edit_simple_template_layout_data = array(
		...
		"external_template_params" => $set_template["template_params"],
	);
	...
	<div class="current_entity_code hidden">' . str_replace(">", "&gt;", str_replace("<", "&lt;", $obj_data["code"])) . '</div>';
...
function getAdvancedSettingsHtml($EVC, $advanced_settings) {
	$is_parse_html = $advanced_settings["parse_full_html"] || $advanced_settings["parse_regions_html"];
	...
	<div class="parser">
		...
		<div class="parse_html">
			...
			<select name="parse_html" onChange="onChangeParseHtml(this)" title="If active the system will parse the html and add or filter elements according with the options defined below.">
				...
				<option value="1"' . ($advanced_settings["parse_full_html"] ? ' selected' : '') . ' title="Parse Full Page Output Html including the template html. Basically parses the html before it is sent to the client browser.">Parse Full Page Output Html</option>
				<option value="2"' . ($advanced_settings["parse_regions_html"] && !$advanced_settings["parse_full_html"] ? ' selected' : '') . '>Only Parse Page Regions Html</option>
			</select>
		...
		<div class="execute_sla"' . $style_display_none . '>
			...
			<select name="execute_sla" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will execute the resources and save them, so we can use them later on through the hash-tags, PTL or directly in the html..."' . $disabled . '>
				<option value="0"' . ($advanced_settings["execute_sla"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["execute_sla"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["execute_sla"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="parse_hash_tags"' . $style_display_none . '>
			...
			<select name="parse_hash_tags" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will replace all hash-tags with the correspondent data from the resources."' . $disabled . '>
				<option value="0"' . ($advanced_settings["parse_hash_tags"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["parse_hash_tags"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["parse_hash_tags"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="parse_ptl"' . $style_display_none . '>
			...
			<select name="parse_ptl" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will convert the PTL code into HTML."' . $disabled . '>
				<option value="0"' . ($advanced_settings["parse_ptl"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["parse_ptl"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["parse_ptl"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="add_my_js_lib"' . $style_display_none . '>
			...
			<select name="add_my_js_lib" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will add the MyJSLib.js file."' . $disabled . '>
				<option value="0"' . ($advanced_settings["add_my_js_lib"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["add_my_js_lib"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["add_my_js_lib"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="add_widget_resource_lib"' . $style_display_none . '>
			...
			<select name="add_widget_resource_lib" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will add the MyWidgetResourceLib.js and MyWidgetResourceLib.css files."' . $disabled . '>
				<option value="0"' . ($advanced_settings["add_widget_resource_lib"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["add_widget_resource_lib"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["add_widget_resource_lib"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="filter_by_permission"' . $style_display_none . '>
			...
			<select name="filter_by_permission" title="\'Auto\': means that the system will try to find if the html needs to be parsed. If active the system will filter (show, hide or remove) the html elements according with the permission defined in each element."' . $disabled . '>
				<option value="0"' . ($advanced_settings["filter_by_permission"] === false ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["filter_by_permission"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["filter_by_permission"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="include_blocks_when_calling_resources"' . $style_display_none . '>
			...
			<select name="include_blocks_when_calling_resources" title="If active the system will include and execute the blocks when a resource gets called."' . $disabled . '>
				<option value="0"' . (!$advanced_settings["include_blocks_when_calling_resources"] ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["include_blocks_when_calling_resources"] === true ? ' selected' : '') . '>Yes</option>
			</select>
		...
		<div class="init_user_data"' . $style_display_none . '>
			...
			<select name="init_user_data" title="If active the system will get the logged user data."' . $disabled . '>
				<option value="0"' . (!$advanced_settings["init_user_data"] ? ' selected' : '') . '>No</option>
				<option value="1"' . ($advanced_settings["init_user_data"] === true ? ' selected' : '') . '>Yes</option>
				<option value=""' . ($advanced_settings["init_user_data"] === null ? ' selected' : '') . '>Auto</option>
			</select>
		...
		<div class="maximum_usage_memory"' . $style_display_none . '>
			...
			<input name="maximum_usage_memory" placeHolder="empty for default: ' . $EVC->getCMSLayer()->getCMSPagePropertyLayer()->getMaximumUsageMemory() . ' bytes" value="' . $advanced_settings["maximum_usage_memory"] . '"' . $disabled . ' />
		...
		<div class="maximum_buffer_chunk_size"' . $style_display_none . '>
			...
			<input name="maximum_buffer_chunk_size" placeHolder="empty for default: ' . $EVC->getCMSLayer()->getCMSPagePropertyLayer()->getMaximumBufferChunkSize() . ' bytes" value="' . $advanced_settings["maximum_buffer_chunk_size"] . '"' . $disabled . ' />
	...
	<div class="cache">
		...
		<div class="cache_page">
			...
			<input type="checkbox" name="cache_page" value="1" onChange="onChangeCacheOption(this)"' . ($advanced_settings["cache_page"] ? ' checked' : '') . ' />
			<input name="cache_page_ttl" placeHolder="TTL in secs" value="' . $advanced_settings["cache_page_ttl"] . '"' . ($advanced_settings["cache_page"] ? '' : ' disabled') . ' />
		</div>
		
		<div class="cache_dispatcher">
			<label>Cache Dispatcher: </label>
			<input type="checkbox" name="cache_dispatcher" value="1" onChange="onChangeCacheOption(this)"' . ($advanced_settings["cache_page"] ? ' checked' : '') . ' />
			<input name="cache_dispatcher_ttl" placeHolder="TTL in secs" value="' . $advanced_settings["cache_dispatcher_ttl"] . '"' . ($advanced_settings["cache_page"] ? '' : ' disabled') . ' />
		</div>
	</div>
...
function getTemplatesHtml($set_template, $selected_template, $available_templates, $installed_wordpress_folders_name) {
	if (is_array($set_template["template_params"])) {
		$selected_template_params_type = is_array($set_template["template_params"]["type"]) ? $set_template["template_params"]["type"]["value"] : null; //used in UI
		
		if ($selected_template_params_type == 'project') {
			$selected_template_params_external_template = is_array($set_template["template_params"]["template_id"]) ? $set_template["template_params"]["template_id"]["value"] : null;
			$selected_template_params_external_project_id = is_array($set_template["template_params"]["external_project_id"]) ? $set_template["template_params"]["external_project_id"]["value"] : null;
			$selected_template_params_keep_original_project_url_prefix = is_array($set_template["template_params"]["keep_original_project_url_prefix"]) ? $set_template["template_params"]["keep_original_project_url_prefix"]["value"] : null;
		}
		else if ($selected_template_params_type == 'block') {
			$selected_template_params_external_template = is_array($set_template["template_params"]["block_id"]) ? $set_template["template_params"]["block_id"]["value"] : null;
			$selected_template_params_external_project_id = is_array($set_template["template_params"]["external_project_id"]) ? $set_template["template_params"]["external_project_id"]["value"] : null;
		}
		else if ($selected_template_params_type == 'wordpress_template') {
			$selected_template_params_external_template = is_array($set_template["template_params"]["url_query"]) ? $set_template["template_params"]["url_query"]["value"] : null;
			$selected_template_params_wordpress_installation_name = is_array($set_template["template_params"]["wordpress_installation_name"]) ? $set_template["template_params"]["wordpress_installation_name"]["value"] : null;
		}
		else if ($selected_template_params_type == 'url') {
			$selected_template_params_url = is_array($set_template["template_params"]["url"]) ? $set_template["template_params"]["url"]["value"] : null;
		}
		
		$selected_template_params_cache_ttl = is_array($set_template["template_params"]["cache_ttl"]) ? $set_template["template_params"]["cache_ttl"]["value"] : null;
	}
	...
	<div class="external_template_params advanced_option"' . (!$selected_template_params_type ? ' style="display:none"' : '') . '>
		<div class="external_template_type">
			...
			<select name="type" onChange="onChangeExternalTemplateType(this)">
				...
				<option value="wordpress_template"' . ($selected_template_params_type == 'wordpress_template' ? ' selected' : '') . '>WordPress Template</option>
			</select>
		</div>
