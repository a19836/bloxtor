...
if (empty($admin_uis_count)) {
	...
	$default_admin_type = !empty($is_admin_ui_simple_allowed) ? "simple" : (
		!empty($is_admin_ui_citizen_allowed) ? "citizen" : (
			!empty($is_admin_ui_low_code_allowed) ? "low_code" : (
...
<ul>
	...
	' . (!empty($is_admin_ui_low_code_allowed) ? '<li class="ui low_code_admin_ui" onClick="document.location=\'' . $project_url_prefix . 'admin?admin_type=low_code\'">
		<div class="icon photo"></div>
