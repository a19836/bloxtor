...
if ($path) {
	...
	if ($PEVC) {
		...
		if (file_exists($file_path)) {
			...
			if (!empty($selected_db_vars["db_brokers_drivers"])) //not sure if we should be doing this filter... Note that the $db_drivers will appear in the "db driver combox/select field" in the widgets properties of the LayoutUIEditor.
				...
				switch ($file_type) {
					case "edit_entity":
						...
						if ($edit_entity_type == "simple") {
							if (empty($P->settings["presentation_templates_path"])) //used in edit_entity_simple.php
							...
							$layer_default_template = isset($vars["project_default_template"]) ? $vars["project_default_template"] : null;
							...
							$template_contents = CMSPresentationLayerHandler::getSetTemplateCode($PEVC, $is_external_template, $selected_or_default_template, $selected_template_params, $includes);
							...
							if (!$edit_entity_simple_hard_code && !CMSPresentationLayerHandler::checkIfEntityCodeContainsSimpleUISettings($obj_data["code"], $selected_template, $includes, $regions_blocks, $selected_project_id)) {
								header("Location: $edit_entity_advanced_url");
								echo "<script>document.location = '$edit_entity_advanced_url';</script>";
								die();
							}
							...
							$sla_settings = isset($sla_settings[0]["sla_settings"]) ? (isset($sla_settings[0]["sla_settings"]["key"]) ? array($sla_settings[0]["sla_settings"]) : $sla_settings[0]["sla_settings"]) : null;
							...
							//PREPARING VIEW
							if (empty($P->settings["presentation_views_path"]))
								launch_exception(new Exception("'PresentationLayer->settings[presentation_views_path]' cannot be undefined!"));
							
							$project_with_auto_view = isset($GLOBALS["project_with_auto_view"]) ? $GLOBALS["project_with_auto_view"] : null;
							...
							$allowed_tasks_tag = array(
								...
								"setblockparams", "settemplateregionblockparam", "includeblock", "addregionhtml", "addregionblock", "rendertemplateregion", "settemplateparam", "gettemplateparam", "resetregionblockjoinpoints"
							);
					...
					case "edit_view": 
						...
						if ($edit_view_type == "simple") {
							...
							$allowed_tasks_tag = array(
								...
								"inlinehtml", "createform",
								"setblockparams", "settemplateregionblockparam", "includeblock", "addregionhtml", "addregionblock", "rendertemplateregion", "settemplateparam", "gettemplateparam", "setpresentationview", "addpresentationview", "setpresentationtemplate"
							);
					...
					case "edit_template": 
						...
						if ($edit_template_type == "simple") {
							...
							$selected_template = isset($path_info["extension"]) ? str_replace("." . $path_info["extension"], "", $selected_template) : $selected_template;
					...
					case "create_block":
						...
						foreach ($all_loaded_modules as $module_id => $loaded_module) {
							if (!empty($project_loaded_modules[$module_id]) && empty($loaded_module["is_hidden_module"])) {
								if (!empty($project_loaded_modules[$module_id]["path"]) && CMSModuleEnableHandler::isModuleEnabled($project_loaded_modules[$module_id]["path"])) {
					...
					case "edit_block": 
						...
						if ($edit_block_type == "simple") {
							...
							if (!empty($obj_data["code"])) {
								...
								$first_block_param = isset($block_params[0]["block"]) ? $block_params[0]["block"] : null;
								$first_block_param_type = isset($block_params[0]["block_type"]) ? $block_params[0]["block_type"] : null;
								$raw_block_id = PHPUICodeExpressionHandler::getArgumentCode($first_block_param, $first_block_param_type);
								$block_join_points = CMSPresentationLayerHandler::getFileAddBlockJoinPointsListByBlock($block_path);
								$block_join_points = isset($block_join_points[$raw_block_id]) ? $block_join_points[$raw_block_id] : null;
								...
								preg_match_all('/([ ]*)->([ ]*)getBlockIdFromFilePath([ ]*)\(([ ]*)__FILE__([ ]*)\)([ ]*)$/iu', $first_block_param, $matches, PREG_PATTERN_ORDER); //'/u' means converts unicode.
								
								if (empty($block_params[0]["block_type"]) && !empty($matches[0][0])) {
									$block_params[0]["block"] = $block_id;
									$block_params[0]["block_type"] = "string";
								}
								else if ($block_id != $first_block_param) 
									$hard_coded = true;
								
								//echo "<pre>";print_r($block_params);echo "</pre>";die();
								
								$module_id = isset($block_params[0]["module_type"]) && $block_params[0]["module_type"] == "string" ? $block_params[0]["module"] : "";
								
								$block_settings = isset($block_params[0]["block_settings"]) ? (isset($block_params[0]["block_settings"]["key"]) ? array($block_params[0]["block_settings"]) : $block_params[0]["block_settings"]) : null;
							}
							...
							if ($module_id) {
								...
								$module = isset($loaded_modules[$module_id]) ? $loaded_modules[$module_id] : null;
								...
								if ($module) {
									...
									$module["enabled"] = isset($project_loaded_module["path"]) ? CMSModuleEnableHandler::isModuleEnabled($project_loaded_module["path"]) : false;
									$module["join_points"] = isset($project_loaded_module["join_points"]) ? $project_loaded_module["join_points"] : null;
									$module["module_handler_impl_file_path"] = isset($project_loaded_module["module_handler_impl_file_path"]) ? $project_loaded_module["module_handler_impl_file_path"] : null;
									...
									$module_group_id = isset($module["group_id"]) ? $module["group_id"] : null;
