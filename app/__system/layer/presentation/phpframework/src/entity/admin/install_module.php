...
$LayoutTypeProjectHandler->filterPresentationLayerProjectsByUserAndLayoutPermissions($projects, $filter_by_layout, UserAuthenticationHandler::$PERMISSION_BELONG_NAME, array(
	...
	if ($_POST && ($_FILES["zip_file"] || $_POST["zip_url"])) {
		...
		$selected_db_driver = $_POST["db_driver"];
		
		$pre_init_configs = array();
		foreach ($projects as $project_name => $project) {
			if (empty($selected_project) || $project_name == $selected_project) {
				if ($project["files"]["pre_init_config.php"] || $project["files"]["pre_init_config"]) {
					$fp = $project["files"]["pre_init_config.php"] ? $project["files"]["pre_init_config.php"] : $project["files"]["pre_init_config"];
					$file_path = substr($project["path"], 0, - strlen($project_name)) . $fp["path"];
		...
		if ($_POST["zip_url"]) {
			$index = $_FILES["zip_file"] && $_FILES["zip_file"]["name"] ? max(array_keys($_FILES["zip_file"]["name"])) + 1 : 0;
			...
			foreach ($_POST["zip_url"] as $zip_url) 
				if (trim($zip_url)) {
					...
					if ($downloaded_file && stripos($downloaded_file["type"], "zip") !== false) {
						...
						$_FILES["zip_file"]["name"][$index] = $downloaded_file["name"];
						$_FILES["zip_file"]["type"][$index] = $downloaded_file["type"];
						$_FILES["zip_file"]["tmp_name"][$index] = $downloaded_file["tmp_name"];
						$_FILES["zip_file"]["error"][$index] = $downloaded_file["error"];
						$_FILES["zip_file"]["size"][$index] = $downloaded_file["size"];
		...
		if ($_FILES["zip_file"]) {
			...
			if (!$exists) {
				...
				ksort($_FILES["zip_file"]["type"]);
				ksort($_FILES["zip_file"]["tmp_name"]);
				ksort($_FILES["zip_file"]["error"]);
				ksort($_FILES["zip_file"]["size"]);
				//echo "<pre>";print_r($_POST);print_r($_FILES);die();
				...
				foreach ($right_order as $file_name)
					foreach ($_FILES["zip_file"]["name"] as $i => $name) {
						if (pathinfo($name, PATHINFO_FILENAME) == $file_name) {
							...
							$new_files["zip_file"]["type"][] = $_FILES["zip_file"]["type"][$i];
							$new_files["zip_file"]["tmp_name"][] = $_FILES["zip_file"]["tmp_name"][$i];
							$new_files["zip_file"]["error"][] = $_FILES["zip_file"]["error"][$i];
							$new_files["zip_file"]["size"][] = $_FILES["zip_file"]["size"][$i];
				...
				foreach ($new_files["zip_file"]["name"] as $i => $name)
					if (trim($name)) {
						...
						$dest_folder_path = CMSModuleInstallationHandler::getTmpFolderPath();
						
						if (!$dest_folder_path)
							$error_message = "Error: trying to create tmp folder to upload '$name' file!";
						else {
							$zipped_file_path = $dest_folder_path . $name;
							$dest_file_path = substr($zipped_file_path, 0, -4) . "/";
							$extension = strtolower( pathinfo($name, PATHINFO_EXTENSION) );
							$module_id = pathinfo($zip_file["name"], PATHINFO_FILENAME);
							
							if ($extension != "zip") {
								$status = false;
								$messages[$module_id][$project_name][] = array("msg" => "STATUS: FALSE: File '$name' must be a zip file!", "type" => "alert");
							}
							else {
								...
								if ($continue) {
									...
									//unzip
									$unzipped_module_path = CMSModuleInstallationHandler::unzipModuleFile($zipped_file_path, $dest_file_path);
									...
									if ($unzipped_module_path) {
										...
										//set new module id
										if ($info && $info["tag"] && $module_id != $info["tag"])
											$module_id = $info["tag"];
										...
										foreach ($pre_init_configs as $project_name => $pre_init_config) {
											...
											try {
												...
												if ($messages[$module_id][$project_name])
													$messages[$module_id][$project_name][] = array("msg" => "GLOBAL SELECTED DB DRIVER: " . $GLOBALS["default_db_driver"], "type" => "info");
											}
											catch(Exception $e) {
												//set message
												$status = false;
									...
									CMSModuleUtil::deleteFolder($unzipped_module_path);
									unlink($zipped_file_path);
								}
								else {
									$status = false;
									$messages[$module_id][$project_name][] = array("msg" => "STATUS: FALSE: File '$name' not uploaded!", "type" => "error");
								}
							}
		...
		$layer_bean_folder_name = WorkFlowBeansFileHandler::getLayerBeanFolderName($user_beans_folder_path . $bean_file_name, $bean_name, $user_global_variables_file_paths);
		...
		$PHPVariablesFileHandler = new PHPVariablesFileHandler(array($user_global_variables_file_paths, $PEVC->getConfigPath("pre_init_config")));
		$PHPVariablesFileHandler->startUserGlobalVariables();
