...
*/
		...
		AnimEvent = function(n) {
			...
			var r = 500,
				o = [],
				i = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || function(n) {
					return setTimeout(n, 1e3 / 60)
				},
				u = window.cancelAnimationFrame || window.mozCancelAnimationFrame || window.webkitCancelAnimationFrame || window.msCancelAnimationFrame || function(n) {
			...
			function c() {
				var n = void 0,
					e = void 0;
				l && (u.call(window, l), l = null), o.forEach(function(e) {
					var t;
					(t = e.event) && (e.event = null, e.listener(t), n = !0)
				}), n ? (a = Date.now(), e = !0) : Date.now() - a < r && (e = !0), e && (l = i.call(window, c))
			}
			...
			var d = {
				...
				remove: function(n) {
					var e;
					(e = f(n)) > -1 && (o.splice(e, 1), !o.length && l && (u.call(window, l), l = null))
				}
			};
			e.default = d
		}]).default,
	...
	function bBox2PathData(bBox) {
		...
		return [{
				type: 'M',
				values: [bBox.left, bBox.top]
			},
			{
				type: 'L',
				values: [right, bBox.top]
			},
			{
				type: 'L',
				values: [right, bottom]
	...
	function getBodyOffset(window) {
		...
		if (stylesBody.position !== 'static') {
			...
			bodyOffset.x -= [stylesHtml.marginLeft, stylesHtml.borderLeftWidth, stylesHtml.paddingLeft,
				stylesBody.marginLeft, stylesBody.borderLeftWidth
			].reduce(sumProps, 0);
			bodyOffset.y -= [stylesHtml.marginTop, stylesHtml.borderTopWidth, stylesHtml.paddingTop,
	...
	function updatePosition(props) {
		...
		// Decide each socket
		(function() {
			var socketXYsWk, socketsLenMin = -1,
				iFix, iAuto;
		...
		if (curStats.position_path !== aplStats.position_path ||
			curStats.position_lineStrokeWidth !== aplStats.position_lineStrokeWidth || [0, 1].some(function(i) {
						...
						curSocketXYSE.forEach(function(socketXY, i) {
							...
							(function(dirLen) {
								...
								len = dirLen[1];
									...
									len[1] > 0 ? [DIR_DOWN, len[1]] : [socketXY.socketId, 0] // (0, 0)
								) :
			...
			(function() {
				...
				curStats.position_plugOverheadSE.forEach(function(plugOverhead, i) {
					var start = !i,
						pathPoints, iSeg, point, sp, cp, angle, len,
	...
	EFFECTS = {
		...
		gradient: {
			...
			update: function(props) {
				...
				point = pathList[0][0];
				curStats.gradient_pointSE[0] = {
					x: point.x,
					y: point.y
				}; // first point of first seg
				pathSeg = pathList[pathList.length - 1];
				point = pathSeg[pathSeg.length - 1];
				curStats.gradient_pointSE[1] = {
					x: point.x,
					y: point.y
				}; // last point of last seg
	...
	Object.keys(EFFECTS).forEach(function(effectName) {
		...
		effectStats[effectName + '_enabled'] = {
			iniValue: false
		};
		effectStats[effectName + '_options'] = {
			hasProps: true
		};
		if (effectConf.anim) {
	...
	LeaderLine.prototype.position = function() {
		this.updated = update(insProps[this._id], {
			position: true
		});
		return this;
	};
					...
					switch (attachProps.shape) {
							...
							(function() {
								...
								if (radius) {
									...
									points = [{
											...
											y: areaBBox.top + offsetC
										}, // 0 left-top-start
										{
											x: areaBBox.left + offsetC,
											y: areaBBox.top - padding
										}, // 1 left-top-end
										{
									...
									curStats.pathListRel.push([points[2], {
											x: points[2].x + cpR,
											y: points[2].y
										},
										{
									...
									curStats.pathListRel.push([points[4], {
											x: points[4].x,
											y: points[4].y + cpR
										},
										{
									...
									curStats.pathListRel.push([points[6], {
											x: points[6].x - cpR,
											y: points[6].y
										},
										{
						...
						case 'circle':
							(function() {
								...
								points = [{
										x: areaBBox.left - paddingX,
										y: areaBBox.top - paddingY
									}, // left-top
									{
						...
						case 'polygon':
							(function() {
								...
								attachProps.points.forEach(function(point) {
									...
									if (curPoint) {
										curStats.pathListRel.push([curPoint, {
											x: x,
											y: y
										}]);
									} else {
			...
			init: function(attachProps, attachOptions) {
				...
				attachProps.updateStartOffset = function(props) {
					var curStats = attachProps.curStats,
						aplStats = attachProps.aplStats,
						llStats = props.curStats,
						pathLenAll, plugBackLen, startOffset;
					// It's not ready yet.
