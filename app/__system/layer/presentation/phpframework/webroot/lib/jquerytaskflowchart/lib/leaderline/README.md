...
2- go to new folder
	cd leader-line/
3- install node package by running:
	npm install
	
	Note that npm version is 6.13.4
	If not you can always execute:
		nvm install 8
		nvm use 8
4- bkp leader-line.min.js
	cp leader-line.min.js leader-line.min.js.bkp
5- Edit Gruntfile.js and change line: 47, so it generates the unminified leader-line.min.js file.
	From:
		return uglify.minify(content).code;
	To:
		return content;
6- run grunt so it can rebuild the files based in Gruntfile.js
	grunt
7- check the leader-line.min.js and rename it to leader-line.js
	mv leader-line.min.js leader-line.js
8- rename the leader-line.min.js.bkp to leader-line.min.js
	mv leader-line.min.js.bkp leader-line.min.js
9- you should have the listed files:
	leader-line.js (unminified version)
	leader-line.min.js (minified version)
10- Then make your changes in js files
