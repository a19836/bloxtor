...
trait DBSQLConverter { 
	...
	public static function buildDefaultTableInsertSQL($table_name, $attributes, $options = false) {
		...
		if ($table_name && $attributes) {
			...
			foreach($attributes as $key => $value) {
				...
				$sql_values .= (strlen($sql_values) ? ", " : "") . self::createBaseExprValue($value);
			}
	...
	public static function buildDefaultTableUpdateSQL($table_name, $attributes, $conditions = false, $options = false) {
		...
		if ($table_name && $attributes) {
			...
			if ($sql_conditions || $all) {
				...
				foreach($attributes as $key => $value)
					$sql_attrs .= ($sql_attrs ? ", " : "") . SQLQueryHandler::getParsedSqlColumnName($key) . "=" . self::createBaseExprValue($value);
	...
	public static function buildDefaultTableFindRelationshipSQL($table_name, $rel_elm, $parent_conditions = false, $options = false) {
		...
		if ($table_name && $rel_elm) {
			...
			$sql_conditions = self::getSQLRelationshipConditions($conditions, $table_name, $parent_conditions);
			$sql_conditions .= $extra_sql_conditions ? ($sql_conditions ? " AND " : "") . $extra_sql_conditions : "";
			...
			$sql .= " FROM " . SQLQueryHandler::getParsedSqlTableName($table_name) . " " . self::getSQLRelationshipJoins($keys, $table_name);
	...
	*/
	...
	public static function getSQLRelationshipConditions($conditions, $table_name = false, $parent_conditions = false) {
		$sql = "";
		
		if(is_array($parent_conditions)) 
			$sql .= ($sql ? " AND " : "") . self::getSQLConditions($parent_conditions, null, $table_name);
			/*foreach($parent_conditions as $key => $value) 
				$sql .= ($sql ? " AND " : "") . self::prepareTableAttributeWithFunction($key, $table_name) . "=" . self::createBaseExprValue($value);*/
		...
		if (!$is_numeric_array) //if associative array
			$sql .= ($sql ? " AND " : "") . self::getSQLConditions($conditions, null, $table_name);
		else
			for ($i = 0; $i < $t; $i++) {
				...
				if (is_array($condition)) {
					...
					if ($column) {
						...
						if (isset($condition["value"])) {
							...
							$sql .= ($sql ? " AND " : "") . self::getSQLConditions($cond, null, $table);
							//$sql .= ($sql ? " AND " : "") . self::prepareTableAttributeWithFunction($column, $table) . " {$operator} " . self::createBaseExprValue($value);
						}
					}
				}
				else
	...
	*/
	...
	/*
	  getSQLRelationshipJoins: Parses the following:
		...
		- right
	*/
	protected static function getSQLRelationshipJoins($keys, $table_name) {
		$joins = array();
	...
	*/
	public static function getSQLConditions($conditions, $join = false, $key_table_name = "") {
		$sql = "";
		...
		if (is_array($conditions)) {
			...
			foreach ($conditions as $key => $value) {
				...
				if ($ukey == "AND" || $ukey == "OR" || (is_numeric($key) && is_array($value))) {
					$sub_sql = is_array($value) ? self::getSQLConditions($value, $ukey, $key_table_name) : (is_string($value) && $value ? $value : "");
						...
						foreach ($value as $v) {
							...
							if (is_array($v)) {
								...
								if ($operator == "in" || $operator == "not in")
									$c .= "$key_str $operator " . self::createBaseExprValueForOperatorIn($val);
								else if ($operator == "is" || $operator == "is not")
									$c .= "$key_str $operator " . self::createBaseExprValueForOperatorIs($val);
								else
									$c .= "$key_str $operator " . self::createBaseExprValue($val);
							}
							else
								$c .= "$key_str = " . self::createBaseExprValue($v);
						}
						$sql .= $c;
					}
					else
						$sql .= "$key_str = " . self::createBaseExprValue($value);
				}
			}
		}
	...
	public static function createBaseExprValueForOperatorIn($value) {
		$create_expr_value_func = function($v) {
			return self::createBaseExprValue($v);
		};
		
		return SQLQueryHandler::createBaseExprValueForOperatorIn($value, $create_expr_value_func);
	}
	
	public static function createBaseExprValueForOperatorIs($value) {
		$create_expr_value_func = function($v) {
			return self::createBaseExprValue($v);
		};
