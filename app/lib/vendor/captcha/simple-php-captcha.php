...
function generate_simple_captcha_image($captcha_config) {
	...
	if (function_exists('imagefilter')) {
		...
		imagefilter($captcha, IMG_FILTER_EMBOSS);
		//imagefilter($captcha, IMG_FILTER_NEGATE);
	}
	
	//write lines
	for ($i = 0; $i < 70; $i += 10)
		imageline($captcha, 0, 0 + $i, $box_width, $box_height - $i, $color);
	
	$scale = 2;
	$line_width = 2;
	$x1 = $box_width*$scale*.15;
	$x2 = $box_width;
	$y1 = rand($box_height*$scale*.40, $box_height*$scale*.65);
	$y2 = rand($box_height*$scale*.40, $box_height*$scale*.65);
	$width = $line_width/2*$scale;

	for ($i = $width*-1; $i <= $width; $i++) {
	  imageline($captcha, $x1, $y1+$i, $x2, $y2+$i, $color);
	  imageline($captcha, $x1 - 50, $y1+$i - 50, $x2, $y2+$i, $color);
	}
