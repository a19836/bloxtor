...
class EncodingFactory
{
	...
	public static function registerModelPrefixToEncoding($modelPrefix, $encodingName)
	{
	...
	public static function registerEncoding($encodingName, $constructor)
	{
	...
	public static function createByModelName($modelName)
	{
		$encodingName = null;
		if (array_key_exists($modelName, self::$modelToEncoding)) {
			$encodingName = self::$modelToEncoding[$modelName];
		} 
	else {
			...
			foreach (self::$modelPrefixToEncoding as $prefix => $targetEncodingName) {
		...
		return self::createByEncodingName($encodingName);
	}

	public static function createByEncodingName($encodingName)
	{
		if (array_key_exists($encodingName, self::$encodingInstance)) {
			return self::$encodingInstance[$encodingName];
		}
		...
		self::$encodingInstance[$encodingName] = $encoding;
	...
	{
		if (!is_null(self::$encodingConstructors)) {
			return;
		}
